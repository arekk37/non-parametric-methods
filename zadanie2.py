...
SampleSize = [i for i in range(1, 100, 50)]
print(SampleSize)
...
for size in SampleSize:
    ...
    for df_x in df:
        ...
        for i in range(test_range):
            ...
            # wyczytałem, że chi2 nie powinien być używany do jednostajnego rozkladu, a można to zrobićw taki sposób, że wg rozkładu który nam wyszedl przeliczamy to na zmienne dyskretne tzn dzielimy to na przedziły (ponoć dobrą metodą jest pierwiastek z liczebnosci proby) i liczymy ile z nich wpada do naszego przedziału i wtedy możemy obliczyć z łatwościa test 
            bins = np.linspace(0, 1, num=num_bins+1)
            counts, _ = np.histogram(data_pit, bins=bins)

            # Obliczanie oczekiwanych częstości dla rozkładu jednostajnego
